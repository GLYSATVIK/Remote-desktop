...
class ArcaneServer:
    ...
    def handle_client(self, conn):
        try:
            ...
            # Let's go with:
            # 1. Server sends "ArcaneServer"
            # 2. Client sends Password
            # 3. Server sends "OK"
            
            conn.sendall(b"ArcaneServer\n")
    ...
    def stream_desktop(self, conn):
        ...
        try:
            while True:
                ...
                # Total 13 bytes
                header = struct.pack("IIIB", len(data), 0, 0, 0) # Updated=0 (False) means full frame in my simplified protocol logic? 
                # Wait, in my previous edit to v_desktop.py:
                # if screen_updated (bool(0x1)): read_json...
                # else: read chunk
                # So Updated=0 is correct for frame data.
...
if __name__ == "__main__":
    server = ArcaneServer("password") # Default password
    server.start()
